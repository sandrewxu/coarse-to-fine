...
def main():
    parser = argparse.ArgumentParser(
        description="Create OpenAI Batch API request JSONL files"
    )
    ...
    parser.add_argument("--doc-start", type=int, default=0, help="Start document index (supports negative)")
    ...
    parser.add_argument("--system-prompt", type=str, default=None, help="System prompt name (without .txt)")
    parser.add_argument("--user-prompt", type=str, default=None, help="User prompt name (without .txt)")
    parser.add_argument("--few-shot", type=str, default=None, help="Few-shot examples name (without .jsonl)")
    parser.add_argument("--output-dir", type=Path, default=None, help="Output directory for request JSONL")
    ...
    if not docs_path.exists():
        print(f"Error: Document file not found: {docs_path}", file=sys.stderr)
        sys.exit(1)
    ...
    system_prompt_name = args.system_prompt or batch_config.get("system_prompt", "latent-generation")
    ...
    print(f"Loaded {len(documents)} documents from {docs_path} [{args.doc_start}:{args.doc_end or 'end'}]")
    ...
    if not user_prompt_path.exists():
        print(f"Error: User prompt not found: {user_prompt_path}", file=sys.stderr)
        sys.exit(1)
    ...
    if few_shot_name:
        ...
        if few_shot_path.exists():
            ...
            print(f"Loaded {len(few_shot_messages) // 2} few-shot examples")
        else:
            print(f"Warning: Few-shot file not found: {few_shot_path}", file=sys.stderr)
